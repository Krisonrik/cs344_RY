...
__global__ void naive_normalized_cross_correlation(float* d_response,
    ...
    int template_half_width)
{
  ...
  if (global_index_1d < num_coordinates)
  {
    ...
    for (int y = image_index_2d.y - template_half_height;
         y <= image_index_2d.y + template_half_height;
         y++)
    {
      for (int x = image_index_2d.x - template_half_width;
           x <= image_index_2d.x + template_half_width;
           x++)
      {
          ...
          uchar4>
{
  ...
  {
    return make_uchar4(
        thrust::get<0>(t), thrust::get<1>(t), thrust::get<2>(t), 255);
  }
};
                ...
                const std::string& templateFilename)
{
  ...
  loadImageRGBA(
      templateFilename, &eyeTemplate, &numRowsTemplate, &numColsTemplate);
  ...
  const dim3 gridSize((numColsImg + blockSize.x - 1) / blockSize.x,
                      (numRowsImg + blockSize.y - 1) / blockSize.y,
                      1);
  ...
  naive_normalized_cross_correlation<<<gridSize, blockSize>>>(
      ...
      templateHalfWidth,
      numColsTemplate,
      numRowsTemplate * numColsTemplate,
      r_mean);
  ...
  thrust::transform(d_combined_response.begin(),
                    d_combined_response.end(),
  ...
  cudaMemcpy(*inputVals,
             thrust::raw_pointer_cast(d_combined_response.data()),
             sizeof(unsigned int) * numElem,
