...
#include <string>
#include "hvr/HW3/loadSaveImage.h"
...
__global__ void rgb_to_xyY(float* d_r,
                           float* d_g,
                           float* d_b,
                           float* d_x,
                           float* d_y,
...
__global__ void normalize_cdf(unsigned int* d_input_cdf,
                              float* d_output_cdf,
                              int n)
{
...
__global__ void tonemap(float* d_x,
                        float* d_y,
                        float* d_log_Y,
                        float* d_cdf_norm,
                        float* d_r_new,
...
M3_DLL
void preProcess(float** d_luminance,
                unsigned int** d_cdf,
                size_t* numRows,
                size_t* numCols,
                ...
                const std::string& filename)
{
  ...
  *numCols = numCols__;
  ...
  const dim3 gridSize((numCols__ + blockSize.x - 1) / blockSize.x,
                      (numRows__ + blockSize.y - 1) / blockSize.y,
                      1);
  rgb_to_xyY<<<gridSize, blockSize>>>(d_red,
                                      d_green,
                                      d_blue,
                                      d_x__,
                                      d_y__,
                 ...
                 float max_log_Y)
{
  ...
  tonemap<<<gridSize, blockSize>>>(d_x__,
                                   d_y__,
                                   d_logY__,
                                   d_cdf_normalized,
                                   d_red,
  ...
  checkCudaErrors(cudaMemcpy(
      h_red, d_red, sizeof(float) * numPixels, cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(
      h_green, d_green, sizeof(float) * numPixels, cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(
      h_blue, d_blue, sizeof(float) * numPixels, cudaMemcpyDeviceToHost));
