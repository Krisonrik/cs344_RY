...
#include "hvr/HW6/reference_calc.h"
#include <thrust/host_vector.h>
#include "hvr/HW6/utils.h"
...
{
  unsigned int off =
      interiorPixelList[0].x * numColsSource + interiorPixelList[0].y;
  ...
  for (size_t i = 0; i < interiorPixelList.size(); ++i)
  {
    ...
    f_next[offset] =
        std::min(255.f, std::max(0.f, f_next_val));  // clip to [0, 255]
  }
}
                    ...
                    uchar4* const h_blendedImg)
{
  ...
  // this is a conceptually simple implementation, not a particularly efficient
  // one...
  ...
  {
    mask[i] = (h_sourceImg[i].x + h_sourceImg[i].y + h_sourceImg[i].z < 3 * 255)
                  ? 1
                  : 0;
  }
  ...
  for (size_t r = 1; r < numRowsSource - 1; ++r)
  {
    for (size_t c = 1; c < numColsSource - 1; ++c)
    {
      ...
      {
        if (mask[(r - 1) * numColsSource + c] &&
            mask[(r + 1) * numColsSource + c] &&
  ...
  // next we'll precompute the g term - it never changes, no need to recompute
  // every iteration
  ...
  // for each color channel we'll need two buffers and we'll ping-pong between
  // them
