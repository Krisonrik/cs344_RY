    ...
    for (int c = 0; c < (int)numCols; ++c)
    {
           ...
           ++filter_r)
      {
             ...
             ++filter_c)
        {
          ...
          float image_value =
              static_cast<float>(channel[image_r * numCols + image_c]);
          float filter_value =
              filter[(filter_r + filterWidth / 2) * filterWidth + filter_c +
                     filterWidth / 2];
                          ...
                          const int filterWidth)
{
  ...
  channelConvolution(
      green, greenBlurred, numRows, numCols, filter, filterWidth);
