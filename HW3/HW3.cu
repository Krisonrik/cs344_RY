...
// Kernel for calculating minimum and maximum value of given array (output array from reduction of 1/4 the size of the original array)
...
)
{
    ...
    // use each block as a single bin and process through the entire array to get a single bin value for each block
    for (int i = 0; i < loopCount; i++) {
        if ((tId + i*blockDim.x) < pxCount) {
            if (blockIdx.x == int((d_logLuminance[tId + i*blockDim.x] - min_logLum) / lumRange * numBins)) {
                bin++;
            }
        }
        __syncthreads();
    ...
    const size_t numBins)
{
    ...
    //std::unique_ptr<float[]> h_min_Lums(new float[gridSize]);//std::make_unique<float[]>(gridSize);
    ...
    checkCudaErrors(cudaMalloc((void**)&d_logLum_Min, pxCount * sizeof(float) / 4));
    checkCudaErrors(cudaMalloc((void**)&d_logLum_Max, pxCount * sizeof(float) / 4));
    ...
    findMinMax << <gridSize, blockSize >> > (d_logLuminance, d_logLum_Min, d_logLum_Max, d_min_Lums, d_max_Lums, pxCount);
    ...
    // copy back arrays of minimum and maximum values for further reduction on cpu to a single minimum and maximum
    checkCudaErrors(cudaMemcpy(h_min_Lums, d_min_Lums, gridSize * sizeof(float), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(h_max_Lums, d_max_Lums, gridSize * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    calcHisto << <numBins, blockSize, sharedMemSize >> > (d_histo, d_logLuminance, min_logLum, lumRange, numBins, pxCount);
    cudaDeviceSynchronize();
    ...
    // calculate how many loops the Hillis Steele scan is required for given array size
    int cycles = 0;
    ...
    calcCDF <<<cdfGridSize, blockSize, sharedMemSize >>> (d_cdf, d_histo, numBins, cycles);
    cudaDeviceSynchronize();
