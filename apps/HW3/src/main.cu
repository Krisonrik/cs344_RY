...
int main(int argc, char **argv)
{
  ...
  switch (argc)
  {
    ...
    case 6:
      ...
      break;
    default:
      std::cerr << "Usage: ./HW3 input_file [output_filename] "
  ...
  your_histogram_and_prefixsum(
      d_luminance, d_cdf, min_logLum, max_logLum, numRows, numCols, numBins);
  timer.Stop();
  ...
  checkCudaErrors(cudaMemcpy(h_luminance,
                             d_luminance,
  ...
  referenceCalculation(
      h_luminance, h_cdf, numRows, numCols, numBins, min_logLum, max_logLum);

  checkCudaErrors(cudaMemcpy(
      d_cdf, h_cdf, sizeof(unsigned int) * numBins, cudaMemcpyHostToDevice));
