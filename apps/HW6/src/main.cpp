...
#include "hvr/HW6/compare.h"
#include "hvr/HW6/reference_calc.h"
#include "hvr/HW6/timer.h"
...
void postProcess(const uchar4* const blendedImg,
                 const size_t numRowsDest,
                 const size_t numColsDest,
...
int main(int argc, char** argv)
{
  ...
  switch (argc)
  {
    ...
    case 7:
      ...
      break;
    default:
      std::cerr << "Usage: ./HW6 input_source_file input_dest_filename "
                   "[output_filename] [reference_filename] [perPixelError] "
                   "[globalError]"
                << std::endl;
      exit(1);
  }
  ...
  your_blend(
      h_sourceImg, numRowsSource, numColsSource, h_destImg, h_blendedImg);
  ...
  reference_calc(
      h_sourceImg, numRowsSource, numColsSource, h_destImg, h_reference);

  // save the reference image
  postProcess(h_reference, numRowsSource, numColsSource, reference_file);

  compareImages(
      reference_file, output_file, useEpsCheck, perPixelError, globalError);
